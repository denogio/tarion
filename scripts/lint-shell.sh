#!/usr/bin/env bash
# ShellCheck wrapper for Tarion project
set -euo pipefail

echo "🔍 Running ShellCheck on Tarion scripts..."
echo "=========================================="
...
# This catches both .sh files and shell scripts without extensions (like tarion-*)
SCRIPTS=$(find . -type f -not -path "./.git/*" -not -path "./.bluebuild-scripts_**" -not -path "./node_modules/*" -not -path "./scripts/*" -not -path "./.inspiration/*" -not -path "./.specify/*" -exec file {} \; | grep -i "shell script" | cut -d: -f1)
